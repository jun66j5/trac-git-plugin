...
from tracext.git.git_fs import GitCachedRepository, GitConnector, GitRepository
...
class GitCommandMixin(object):
    ...
    def _git_date_format(self, dt):
        ...
        return '%d %c%02d:%02d' % (to_timestamp(dt), '+-'[secs < 0], hours,
                                   rem / 60)
...
class GitNormalTestCase(BaseTestCase):
    ...
    def _test_on_empty_repos(self, cached_repository):
        ...
        # The following has been fixed in [12720/branches/1.0-stable/trac]
        self.assertRaises(NoSuchChangeset, log_mod.process_request, req)
